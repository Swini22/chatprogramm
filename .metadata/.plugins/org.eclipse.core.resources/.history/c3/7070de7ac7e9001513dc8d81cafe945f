	...
	private void prepareGui() {
		...
		search.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				employees.removeAll();
				if (response.size() > 0) {
					for (EmployeeDepartmentDTO edDTO : response) {
						if (employeesListAlreadyInChat != null) {
							if (!employeesListAlreadyInChat.contains(edDTO.getUser())) {
								if (checkedDepartments.contains(edDTO.getDepartment())) {
									JLabel l = newLabelWithActionListener(edDTO.getUser());
									employees.add(l);
								}else{
									employees.add(new JLabel("no Employees with this name found in this department"));
								}
							}else{
								employees.add(new JLabel("this User is already in the chat"));
							}
						}else {
							if (!username.equals(edDTO.getUser())) {
								if (checkedDepartments.contains(edDTO.getDepartment())) {
									JLabel l = newLabelWithActionListener(edDTO.getUser());
									employees.add(l);
								}else{
								...
								employees.add(new JLabel("thats you mate..."));
							}
						}
					}
				} else {
